...
## Per Feature Aggression
 The table generated by `ClinicalOutcomes` provides a base dataset. To demonstrate per feature aggression via fishers combined statistics we start with a toy dataset.    `PerGeneAggressionTF` generates a numeric aggression value for each gene from this base dataset.  
