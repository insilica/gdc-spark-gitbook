...
The resulting table is below:
  ...
  | stage | lymphatic invasion | metastasis | vascular invasion | % pos. nodes|
  |-------------|-------------|-------------|----|
  | IV | True | True | False | 58% |
  | III | False | True | False | 42% |
  | II | True | True | True | 0% |
  <center>Table 1</center>
## Per Feature Aggression
 The table generated by `ClinicalOutcomes` provides a base dataset. To show per feature aggression via fishers combined statistics we start with a toy dataset. This toy dataset contains genes with high and low aggression numbers according to cancerregulome.  It also contains all the categorical clinical outcomes from table 1. The object `ClinicalOutcomesRegulomeGenes` builds this dataset:
